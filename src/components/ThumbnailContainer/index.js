...
class ThumbnailContainer extends Component {
  state = {thumbnailsList: []}

  renderTabItems = () => {
    const {tabList, activeTab, tabChange} = this.props
    // console.log(tabList)
    return (
      <ul className="tabs-container">
        {tabList.map(eachTab => (
          <li
            key={eachTab.tabId}
            className="tab-item"
            onClick={() => this.getThumbnails(eachTab.tabId)}
          >
            {eachTab.displayText}
          </li>
        ))}
  ...
  getThumbnails = activeTab => {
    const list = imagesList.filter(each => each.category === activeTab)
    this.setState({thumbnailsList: list})
  }

  compareImages = category => {
      ...
      <ul className="thumbnails-list">
        {thumbnailsList.map(each => (
          <img
            ...
            onClick={() => this.compareImages(each.category)}
          />
        ))}
      </ul>
    )
